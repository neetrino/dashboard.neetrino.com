...
class Neetrino_Admin {
    ...
    private function init_hooks() {
        ...
        // Хуки для обработки действий обновления
        add_action('admin_post_neetrino_check_update', [$this, 'handle_check_update']);
    ...
    public function handle_check_update() {
        ...
        if (!current_user_can('administrator') || !check_admin_referer('neetrino_check_update')) {
            wp_die(__('You do not have sufficient permissions to perform this action.', 'neetrino'));
        }
        
        // Очищаем старые флаги и кэш
        delete_transient('neetrino_update_available');
        delete_option('neetrino_update_info');
        wp_clean_plugins_cache(false);
        delete_site_transient('update_plugins');
        
        // Используем метод из класса Neetrino_Plugin_Updater
        $result = $this->updater->check_update_availability();
    ...
    public function handle_direct_update() {
        ...
        if (!get_transient('neetrino_update_available')) {
            $result = [
                'success' => false,
                'message' => __('No updates available at this time.', 'neetrino')
            ];
            set_transient('neetrino_update_result', $result, 30);
            wp_redirect(admin_url('admin.php?page=neetrino_dashboard'));
            exit;
