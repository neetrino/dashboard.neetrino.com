...
class Neetrino_Plugin_Update_Manager {
    ...
    public function perform_plugin_update() {
        error_log('NEETRINO Plugin Update: perform_plugin_update() вызван');
        ...
        if (!is_wp_error($upgraded)) {
            ...
            // Отправляем новую версию на дашборд
            if (class_exists('Neetrino_Registration') && method_exists('Neetrino_Registration', 'push_version_if_changed')) {
                error_log('NEETRINO Plugin Update: Отправляем новую версию на дашборд после обновления');
                
                // Получаем версию из обновленного файла плагина
                $plugin_data = get_plugin_data(NEETRINO_PLUGIN_FILE);
                $new_version = $plugin_data['Version'] ?? 'unknown';
                error_log('NEETRINO Plugin Update: Новая версия из файла: ' . $new_version);
                
                // Отправляем версию с принудительным обновлением
                Neetrino_Registration::push_version_if_changed(true, $new_version);
            } else {
                error_log('NEETRINO Plugin Update: Класс Neetrino_Registration не найден для отправки версии');
            }
            
            error_log('NEETRINO Plugin Update: Обновление успешно завершено');
            return [
                'success' => true,
            ...
            error_log('NEETRINO Plugin Update: Ошибка при обновлении: ' . $upgraded->get_error_message());
            return [
                'success' => false,
